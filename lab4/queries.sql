...
FROM
    (
        employees
        JOIN departments ON employees.managerOf = departments.code
    )
WHERE
...
FROM
    (
        departments
        JOIN projects ON departments.code = managingDept
    )
WHERE
    dept_name = 'research and development';
...
FROM
    (
        (
            ...
            JOIN supervise ON supervise.supervisor = e_supervisor.code
        )
...
FROM
    (
        (
            ...
            JOIN supervise ON supervise.supervised = e_supervised.code
        )
...
FROM
    projects
    JOIN departments ON managingDept = departments.code
WHERE
    dept_name = 'research and development';
...
FROM
    (
        ...
        FROM
            (
                dependants
                JOIN employees ON dependants.employee = employees.code
            )
...
FROM
    (
        ...
        FROM
            ...
            RIGHT JOIN departments ON department = departments.code
        GROUP BY
            departments.code
    ) d
    JOIN departments ON departments.code = d.code
ORDER BY
    dependants_count ASC
FETCH FIRST
    1 ROW ONLY;
...
FROM
    (
        SELECT
            ...
            SUM(NVL(total_hour, 0)) as total_hours
        FROM
            (
                (
                    projectJoin
                    JOIN employees ON projectJoin.employee = employees.code
                )
...
FROM
    (
        SELECT
            project,
            COUNT(employee) as member_count
        FROM
            projectJoin
        GROUP BY
            project
...
FROM
    (
        SELECT
            code,
            sum(NVL(total_hour, 0)) as total_hour
        FROM
            projectJoin
            RIGHT JOIN projects ON code = project
        GROUP BY
...
FROM
    ...
    LEFT JOIN (
        ...
        FROM
            (
                projects
                LEFT JOIN projectJoin ON code = project
            )
        GROUP BY
            code
    ) proj ON proj.code = projects.code
ORDER BY
    member_count ASC
FETCH FIRST
    1 ROW ONLY;
 ...
 */
SELECT
    COUNT(managingDept) as departments_count,
    proj_location
FROM
    projects
GROUP BY
...
FROM
    (
        (
            SELECT
                departments.code as dept,
                COUNT(proj_location) as location_count
            FROM
                projects
                RIGHT JOIN departments ON departments.code = managingDept
            GROUP BY
                departments.code
        ) dept_location
...
FROM
    (
        ...
        FROM
            employees
            LEFT JOIN dependants ON employee = employees.code
        GROUP BY
            employees.code
    ) dep
    JOIN employees ON dep.code = employees.code
ORDER BY
    dependants_count DESC
FETCH FIRST
    1 ROW ONLY;
...
FROM
    (
        SELECT
            employees.department AS code
        FROM
            (
                supervise
                JOIN employees ON employees.code = supervised
            )
        MINUS
        SELECT
            code
    ...
    (
        ...
        (
            ...
            FROM
                projectJoin
                JOIN employees ON employee = employees.code
                JOIN projects ON project = projects.code
                AND proj_name = 'Hydroelectric Dam'
                JOIN departments ON department = departments.code
        )
    ) nojoin
    JOIN departments ON nojoin.code = departments.code;
...
FROM
    (
        (
            SELECT
                departments.code,
                COUNT(managingDept) AS projects_count
            FROM
                projects
                RIGHT JOIN departments ON managingDept = departments.code
            GROUP BY
                departments.code
        ) dep
            ...
            FROM
                (
                    SELECT
                        ...
                        COUNT(employees.code) as emp_count
                    FROM
                        departments
                        LEFT JOIN employees ON department = departments.code
                    GROUP BY
                        departments.code
                )
            WHERE
...
FROM
    (
        ...
        JOIN supervise ON code = supervisor
    )
    JOIN employees ON supervised = employees.code
    JOIN (
        select
            employees.code as emp_code,
            SUM(NVL(total_hour, 0)) as total_hour
